...
function logout() {
    ...
    const token = sessionStorage.getItem("parcelProAdminPresenceToken");
    if (token) {
        navigator.sendBeacon(
...
async function sendConversationImage(sender, email, name, message, file) {
    ...
    if (sender === "admin") {
        body.sessionId = getPresenceSessionId();
        body.token = sessionStorage.getItem("parcelProAdminPresenceToken");
    }
...
function getPresenceSessionId() {
    let id = sessionStorage.getItem("parcelProPresenceSessionId");
    if (!id) {
        id = (crypto.randomUUID ? crypto.randomUUID() : `${Date.now()}-${Math.random()}`).slice(0, 96);
        sessionStorage.setItem("parcelProPresenceSessionId", id);
    }
    return id;
}
...
function connectPresenceEvents(scope) {
    ...
    if (scope === "admin") {
        const token = sessionStorage.getItem("parcelProAdminPresenceToken");
        if (!token) return;
        url += `&sessionId=${encodeURIComponent(getPresenceSessionId())}&token=${encodeURIComponent(token)}`;
    }
